...
import { getRechargeById } from 'apiSdk/recharges';
import { RechargeInterface } from 'interfaces/recharge';

function RechargeViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<RechargeInterface>(
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('recharge', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/recharges/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem
                  label="Recharge Date"
                  text={
                    data?.recharge_date ? format(parseISO(data?.recharge_date as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />

                <FormListItem label="Recharge Status" text={data?.recharge_status} />

                <FormListItem label="Recharge Type" text={data?.recharge_type} />
