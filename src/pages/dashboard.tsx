...
import { getRecharges } from 'apiSdk/recharges';
import { RechargeInterface } from 'interfaces/recharge';
import { getCashbacks } from 'apiSdk/cashbacks';
import { CashbackInterface } from 'interfaces/cashback';
import { getBillPayments } from 'apiSdk/bill-payments';
import { BillPaymentInterface } from 'interfaces/bill-payment';

import { RechargeListPage as ShowCaseList } from 'pages/recharges';

function DashboardPage() {
  const { session } = useSession();

  const rechargeStat = useSWR<PaginatedInterface<RechargeInterface>>(
    () => `/recharges`,
    () => getRecharges({ limit: 1, offset: 0 }),
  );

  const cashbackStat = useSWR<PaginatedInterface<CashbackInterface>>(
    () => `/cashbacks`,
    () => getCashbacks({ limit: 1, offset: 0 }),
  );

  const billPaymentStat = useSWR<PaginatedInterface<BillPaymentInterface>>(
    () => `/bill-payments`,
    () => getBillPayments({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Recharges',
      result: rechargeStat?.data?.totalCount,
      isLoading: rechargeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Cashbacks',
      result: cashbackStat?.data?.totalCount,
      isLoading: cashbackStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bill Payments',
      result: billPaymentStat?.data?.totalCount,
      isLoading: billPaymentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`recharges`} passHref legacyBehavior>
                  <Button
